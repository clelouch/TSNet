...
from model_pvt import Net
from data import get_loader, test_dataset
from utils import clip_gradient, adjust_lr, setup_seed, adjust_learning_rate_poly
from loss import build_loss
import torch.nn.functional as F
from tensorboardX import SummaryWriter
import logging

from options_pvt import opt
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        ...
        for i, (images, gts, gt_edges) in enumerate(train_loader, start=1):
            ...
            pred0, edge0, [pred1, pred2, pred3, pred4], [edge1, edge2, edge3,
                                                         edge4], cropped_mask, cropped_edge = model(images)
            ...
            mask_loss = mask_loss1 + (mask_loss0 + mask_loss2) / 2 + mask_loss3 / 4 + mask_loss4 / 8
            edge_loss = edge_loss1 + (edge_loss0 + edge_loss2) / 2 + edge_loss3 / 4 + edge_loss4 / 8
            ...
            if i % 100 == 0:
                ...
                print(
                    ...
                    ' MaskLoss0: {:0.4f}, MaskLoss1: {:0.4f}, MaskLoss2: {:0.4f}, MaskLoss4: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, eta_str,
                           mask_loss0.data, mask_loss1.data, mask_loss2.data, mask_loss4.data))
                logging.info(
                    '#TRAIN Mask#: {} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], eta: {}, MaskLoss0: {:0.4f}, '
                    'MaskLoss1: {:0.4f}, MaskLoss2: {:0.4f}, MaskLoss4: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, eta_str,
                           mask_loss0.data, mask_loss1.data, mask_loss2.data, mask_loss4.data))
                logging.info(
                    '#TRAIN Edge#: {} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], eta: {}, EdgeLoss0: {:0.4f}, '
                    'EdgeLoss1: {:0.4f}, EdgeLoss2: {:0.4f}, EdgeLoss4: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, eta_str, edge_loss0.data,
                           edge_loss1.data, edge_loss2.data, edge_loss4.data))
                ...
                res = res.data.cpu().numpy().squeeze()
                # res = (res - res.min()) / (res.max() - res.min() + 1e-8)
                writer.add_image('Final_Pred_no_normal', torch.tensor(res), step, dataformats='HW')
                ...
                res = cropped_mask[0][0].clone()
                res = res.data.cpu().numpy().squeeze()
                res = (res - res.min()) / (res.max() - res.min() + 1e-8)
                writer.add_image('Cropped_Mask', torch.tensor(res), step, dataformats='HW')

                res = cropped_edge[0][0].clone()
                res = res.data.cpu().numpy().squeeze()
                res = (res - res.min()) / (res.max() - res.min() + 1e-8)
                writer.add_image('Cropped_Edge', torch.tensor(res), step, dataformats='HW')
                start_time = time.time()
        ...
        if (epoch) % 10 == 0:
            torch.save(model.state_dict(), os.path.join(save_path, 'models', 'TSNet_epoch_{}.pth'.format(epoch)))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), os.path.join(save_path, 'models', 'TSNet_epoch_{}.pth'.format(epoch + 1)))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), os.path.join(save_path, 'models', 'TSNet_epoch_best.pth'))
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.epoch + 1):
        if epoch % opt.decay_epoch == 0:
            ...
            elif opt.adjust == 'poly':
                cur_lr = adjust_learning_rate_poly(optimizer, epoch, opt.epoch + 1, opt.lr, power=0.9)
            else:
